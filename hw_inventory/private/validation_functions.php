<?php
  ...
  function has_unique_board_sn($input_sn, $id="0") {
    global $db;

    $sql = "SELECT * FROM boards ";
    $sql .= "WHERE sn='" . db_escape($db, $input_sn) . "' ";
    $sql .= "AND id != '" . db_escape($db, $id) . "'";

    $board_set = mysqli_query($db, $sql);
    $board_count = mysqli_num_rows($board_set);
    mysqli_free_result($board_set);

    return $board_count === 0;
  }

  function has_unique_dep_name($input_dep, $id="0") {
    global $db;

    $sql = "SELECT * FROM department ";
    $sql .= "WHERE dep='" . db_escape($db, $input_dep) . "' ";
    $sql .= "AND id != '" . db_escape($db, $id) . "'";

    $dep_set = mysqli_query($db, $sql);
    $dep_count = mysqli_num_rows($dep_set);
